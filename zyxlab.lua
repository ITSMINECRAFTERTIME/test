    ...
    local ok, err = xpcall(function()
        ...
        local function styleButton()
            ...
            for _, obj in pairs(button:GetChildren()) do
                if (obj:IsA("TextLabel") or obj:IsA("ImageLabel")) and obj.Name ~= "ZyxPCCrawlLabel" then
                    obj:Destroy()
                end
            end
            ...
            stroke:SetAttribute("ZyxAccentStroke", true)
        ...
        -- Returns true if a ContextActionButton looks like the default crawl button
        local function looksLikeDefaultCrawlButton(child)
            if child.Name ~= "ContextActionButton" then return false end
            if child:GetAttribute("ZyxPCCrawlButton") then return false end
            for _, desc in pairs(child:GetDescendants()) do
                if desc:IsA("TextLabel") or desc:IsA("TextButton") then
                    local txt = tostring(desc.Text or ""):lower()
                    if txt == "c" or txt:find("crawl", 1, true) then
                        return true
                    end
                end
            end
        ...
        -- Destroy any already-present default crawl buttons inside a TouchControlFrame
        local function purgeExistingCrawlButtons(touchFrame)
            for _, child in pairs(touchFrame:GetChildren()) do
                if looksLikeDefaultCrawlButton(child) then
                    pcall(function() child:Destroy() end)
                end
            end
        end

        -- Watch a TouchControlFrame and destroy the default crawl button the instant it appears
        local crawlWatcherConns = {}
        local function watchTouchFrame(touchFrame)
            purgeExistingCrawlButtons(touchFrame)
            local conn = touchFrame.ChildAdded:Connect(function(child)
                -- Give Roblox one frame to finish parenting the button's children
                -- so descendant text is readable, then kill it immediately.
                task.defer(function()
                    if looksLikeDefaultCrawlButton(child) then
                        pcall(function() child:Destroy() end)
                    end
            ...
            table.insert(crawlWatcherConns, conn)
        end

        -- Attach watchers to TouchGui → TouchControlFrame (present or future)
        local function hookTouchGui(touchGui)
            local touchFrame = touchGui:FindFirstChild("TouchControlFrame")
            if touchFrame then
                watchTouchFrame(touchFrame)
            end
            local conn = touchGui.ChildAdded:Connect(function(child)
        ...
        if existingTouchGui then hookTouchGui(existingTouchGui) end
        local touchGuiConn = playerGui.ChildAdded:Connect(function(child)
            if child.Name == "TouchGui" then hookTouchGui(child) end
        end)
        table.insert(crawlWatcherConns, touchGuiConn)
        env.__ZyxCrawlWatcherConns = crawlWatcherConns
        ...
        env.__ZyxPCCrawlMobileThread = task.spawn(function()
            while env.__ZyxPCCrawlMobileActivated do
                ...
                local button = ContextActionService:GetButton(actionName)
                if button then button.Visible = not isRestricted() end

                -- Continuously purge any re-spawned default crawl buttons.
                -- Roblox can recreate the button at any time (respawn, map change, etc.)
                -- so we scan every tick rather than relying only on ChildAdded.
                pcall(function()
...
miscPage:Button({
    Title = "PC Crawl Mobile",
    Desc  = "One-time setup for Shift/L2 crawl plus the mobile C crawl button",
    Text  = "Activate",
    Callback = function()
...
local function setupFeatureSearch(pageRegistry)
    ...
    local items = {
        ...
        {page = "Misc", feature = "PC Crawl Mobile"},
...
do
    ...
    performancePage:Toggle({
        ...
        Value = false,
        Callback = function(v) pf.setRenderQuality(v) end,
    })
    ...
    performancePage:Slider({
        ...
        Rounding = 1,
        Value    = 60,
        Callback = function(v) pf.setFPSCap(v) end,
    })
end
