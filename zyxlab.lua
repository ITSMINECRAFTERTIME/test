...
-- Safe notification wrapper: silently swallows the call if the library
-- version doesn't expose :Notification (prevents nil-call crashes).
local function LibNotify(opts)
    pcall(function() Library:Notification(opts) end)
end
...
local Window
local _winOk, _winErr = pcall(function()
...
if not _winOk or not Window then
    showErrorOnScreen("Library:Window() failed: " .. tostring(_winErr))
    showErrorOnScreen("Fix: Re-run the script. If it persists, the library may have updated.")
    pcall(function()
        local env = (getgenv and getgenv()) or _G
        if env.__ZyxLoadingError then env.__ZyxLoadingError("Window creation failed") end
    end)
    return
end
...
do
    ...
    _G.__ZyxMovementPage:Button({
        ...
        Callback = function()
            ...
            if name then
                LibNotify({ Title = "Orbit target set", Desc = name, Duration = 3, Type = "Success" })
            else
                LibNotify({ Title = "Target not found", Desc = "Try a different name.", Duration = 3, Type = "Warning" })
            end
        end,
    })
    ...
    _G.__ZyxMovementPage:Button({
        ...
        Callback = function()
            ...
            LibNotify({ Title = "Movement reset", Desc = "Movement effects were restored.", Duration = 3, Type = "Success" })
        end,
    })
end
...
local function StartBang()
    ...
    if not target or not root or not hum then
        LibNotify({ Title = "Bang", Desc = "Beast not found!", Duration = 3, Type = "Warning" })
        bangEnabled = false
        return
    end
...
configPage:Button({
    ...
    Callback = function()
        ...
        if name == "" then
            LibNotify({ Title = "Need a name", Desc = "Type a name first.", Duration = 3, Type = "Warning" })
            return
        end
        if name == DEFAULT_CFG then
            LibNotify({ Title = "Reserved", Desc = "Can't overwrite Default.", Duration = 3, Type = "Warning" })
            return
        end
        ...
        if ok then
            ...
            LibNotify({ Title = "Saved", Desc = "'" .. name .. "' is ready to load.", Duration = 3, Type = "Success" })
        else
            LibNotify({ Title = "Save failed", Desc = "Executor blocked the write.", Duration = 4, Type = "Error" })
        end
    end,
})
...
local function applyLoadedConfig(name)
    ...
    if not cfg then
        LibNotify({ Title = "Load failed", Desc = "'" .. name .. "' is missing or corrupt.", Duration = 4, Type = "Error" })
        return
    end
    applyConfigData(cfg)
    ...
    activeConfigName = name
    setLastCfgName(name)
    LibNotify({ Title = "Loaded", Desc = "'" .. name .. "' applied.", Duration = 3, Type = "Success" })
end
...
configPage:Button({
    ...
    Callback = function()
        ...
        if not name or name == DEFAULT_CFG then
            LibNotify({ Title = "Can't overwrite", Desc = "Default is protected.", Duration = 3, Type = "Warning" })
            return
        end
        local ok = writeConfig(name, state, theme)
        if ok then
            ...
            setLastCfgName(name)
            repopulateDropdown(name)
            LibNotify({ Title = "Overwritten", Desc = "'" .. name .. "' now uses current settings.", Duration = 3, Type = "Success" })
        else
            LibNotify({ Title = "Overwrite failed", Desc = "Executor blocked the write.", Duration = 4, Type = "Error" })
        end
    end,
})
...
configPage:Button({
    ...
    Callback = function()
        local name = selectedConfigName
        if not name or name == DEFAULT_CFG then
            LibNotify({ Title = "Can't delete", Desc = "Default is protected.", Duration = 3, Type = "Warning" })
            return
        end
        deleteConfig(name)
        ...
        LibNotify({ Title = "Deleted", Desc = "'" .. name .. "' is gone.", Duration = 3, Type = "Info" })
    end,
})
...
configPage:Button({
    ...
    Callback = function()
        ...
        LibNotify({ Title = "Reset", Desc = "All settings back to defaults.", Duration = 3, Type = "Success" })
    end,
})
